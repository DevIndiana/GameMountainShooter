...
class Level:
    def __init__(self, window: Surface, name: str, game_mode: str, player_score: list[int]):
        # Definindo o timeout com base no nível
        if name == "Level3":  # Verifica se é o nível 3
            self.timeout = TIMEOUT_LEVEL * 2  # Dobro do tempo padrão
        else:
            self.timeout = TIMEOUT_LEVEL  # Tempo padrão para outros níveis
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == EVENT_ENEMY:
                    # Gera inimigos apenas se não estiver no nível 3
                    if self.name != "Level3":  # Assumindo que o nível 3 é chamado "Level3"
                        choice = random.choice(('Enemy1', 'Enemy2'))
                        self.entity_list.append(EntityFactory.get_entity(choice))
                    else:
                        # Adiciona apenas Enemy3 no nível 3
